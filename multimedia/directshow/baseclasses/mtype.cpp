...
{
    // I don't believe we need to check sample size or temporal compression
    // flags, since I think these must be represented in the type, subtype and
    // format somehow. They are pulled out as separate flags so that people who
    // don't understand the particular format representation can still see them,
    // but they should duplicate information in the format block.
...
void WINAPI FreeMediaType(__inout AM_MEDIA_TYPE& mt) {
    if (mt.cbFormat != 0) {
        ...
        mt.cbFormat = 0;     // Strictly unnecessary but tidier
        mt.pbFormat = NULL;  //  ditto
    }
    if (mt.pUnk != NULL) {
        mt.pUnk->Release();
        mt.pUnk = NULL;
...
STDAPI CreateAudioMediaType(const WAVEFORMATEX *pwfx,
                            __out AM_MEDIA_TYPE *pmt, BOOL bSetFormat) {
