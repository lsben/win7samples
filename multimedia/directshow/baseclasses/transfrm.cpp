...
CTransformInputPin::CompleteConnect(IPin *pReceivePin) {
    ...
    if (FAILED(hr)) { return hr; }    return CBaseInputPin::CompleteConnect(pReceivePin);
}
...
// here's the next block of data from the stream.  AddRef it yourself if you
// need to hold it beyond the end of this call.
HRESULT
...
CTransformOutputPin::CompleteConnect(IPin *pReceivePin) {
    ...
    if (FAILED(hr)) { return hr; }    return CBaseOutputPin::CompleteConnect(pReceivePin);
}
